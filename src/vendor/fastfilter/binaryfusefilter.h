...
static inline uint32_t binary_fuse8_hash(uint64_t index, uint64_t hash, const binary_fuse8_t *filter) {
...
static inline bool binary_fuse8_contain(uint64_t key, const binary_fuse8_t *filter) {
	...
	f ^= (uint32_t)filter->Fingerprints[hashes.h0] ^ filter->Fingerprints[hashes.h1] ^ filter->Fingerprints[hashes.h2];
	return f == 0;
}
...
static inline bool binary_fuse8_allocate(uint32_t size, binary_fuse8_t *filter) {
	...
	filter->Fingerprints = (uint8_t *)calloc(filter->ArrayLength, sizeof(uint8_t));
...
static inline bool binary_fuse8_populate(uint64_t *keys, uint32_t size, binary_fuse8_t *filter) {
	if (size != filter->Size) {
		return false;
	}
	...
	for (uint32_t i = size - 1; i < size; i--) {
		...
		filter->Fingerprints[h012[found]] =
		    (uint8_t)((uint32_t)xor2 ^ filter->Fingerprints[h012[found + 1]] ^ filter->Fingerprints[h012[found + 2]]);
	}
	free(alone);
	free(t2count);
	free(reverseH);
...
static inline uint32_t binary_fuse16_hash(uint64_t index, uint64_t hash, const binary_fuse16_t *filter) {
	uint64_t h = binary_fuse_mulhi(hash, filter->SegmentCountLength);
	h += index * filter->SegmentLength;
	// keep the lower 36 bits
	uint64_t hh = hash & ((1ULL << 36U) - 1);
...
// before. For best performance, the caller should ensure that there are not too
// many duplicated keys.
static inline bool binary_fuse16_populate(uint64_t *keys, uint32_t size, binary_fuse16_t *filter) {
	if (size != filter->Size) {
		return false;
	}
	...
	for (uint32_t i = size - 1; i < size; i--) {
		...
		h012[4] = h012[1];
		filter->Fingerprints[h012[found]] =
		    (uint16_t)((uint32_t)xor2 ^ (uint32_t)filter->Fingerprints[h012[found + 1]] ^
...
static inline size_t binary_fuse16_serialization_bytes(binary_fuse16_t *filter) {
	return sizeof(filter->Seed) + sizeof(filter->Size) + sizeof(filter->SegmentLength) +
	       sizeof(filter->SegmentLengthMask) + sizeof(filter->SegmentCount) + sizeof(filter->SegmentCountLength) +
	       sizeof(filter->ArrayLength) + sizeof(uint16_t) * filter->ArrayLength;
}

static inline size_t binary_fuse8_serialization_bytes(const binary_fuse8_t *filter) {
	return sizeof(filter->Seed) + sizeof(filter->Size) + sizeof(filter->SegmentLength) + sizeof(filter->SegmentCount) +
	       sizeof(filter->SegmentCountLength) + sizeof(filter->ArrayLength) + sizeof(uint8_t) * filter->ArrayLength;
}
...
#define XOR_packers(fuse) XOR_bytesf(fuse) XOR_packf(fuse) XOR_unpackf(fuse)

XOR_packers(fuse8) XOR_packers(fuse16)
