...
static inline bool xor8_contain(uint64_t key, const xor8_t *filter) {
	...
	return f == ((uint32_t)filter->fingerprints[h0] ^ filter->fingerprints[h1] ^ filter->fingerprints[h2]);
}
...
static inline bool xor_init_buffer(xor_setbuffer_t *buffer, size_t size) {
	...
	buffer->buffer = (xor_keyindex_t *)malloc((size_t)buffer->slotcount * buffer->slotsize * sizeof(xor_keyindex_t));
...
static inline bool xor16_buffered_populate(uint64_t *keys, uint32_t size, xor16_t *filter) {
	...
	while (stack_size > 0) {
		...
		if (ki.index < blockLength) {
			val ^=
			    (uint32_t)fingerprints1[xor16_get_h1(ki.hash, filter)] ^ fingerprints2[xor16_get_h2(ki.hash, filter)];
		} else if (ki.index < 2 * blockLength) {
			val ^=
			    (uint32_t)fingerprints0[xor16_get_h0(ki.hash, filter)] ^ fingerprints2[xor16_get_h2(ki.hash, filter)];
		} else {
			val ^=
			    (uint32_t)fingerprints0[xor16_get_h0(ki.hash, filter)] ^ fingerprints1[xor16_get_h1(ki.hash, filter)];
		}
...
static inline size_t xor16_serialization_bytes(xor16_t *filter) {
	return sizeof(filter->seed) + sizeof(filter->blockLength) + sizeof(uint16_t) * 3 * (size_t)(filter->blockLength);
}

static inline size_t xor8_serialization_bytes(const xor8_t *filter) {
	return sizeof(filter->seed) + sizeof(filter->blockLength) + sizeof(uint8_t) * 3 * (size_t)(filter->blockLength);
}
...
static inline void xor16_deserialize_no_copy(xor16_t *filter, const char *buffer) {
	memcpy(&filter->seed, buffer, sizeof(filter->seed));
	buffer += sizeof(filter->seed);
	memcpy(&filter->blockLength, buffer, sizeof(filter->blockLength));
	buffer += sizeof(filter->blockLength);
	filter->fingerprints = (uint16_t *)buffer;
}
...
static inline void xor8_deserialize_no_copy(xor8_t *filter, const char *buffer) {
	memcpy(&filter->seed, buffer, sizeof(filter->seed));
	buffer += sizeof(filter->seed);
	memcpy(&filter->blockLength, buffer, sizeof(filter->blockLength));
	buffer += sizeof(filter->blockLength);
	filter->fingerprints = (uint8_t *)buffer;
}
...
#define XOR_packers(xbits) XOR_bytesf(xbits) XOR_packf(xbits) XOR_unpackf(xbits)

XOR_packers(8) XOR_packers(16)
